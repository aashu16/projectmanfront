...
class Tasklist extends Component {
	...
	let request = 'http://api.induago.com/list/'+this.props.taskid;
	let self = this;
	...
	if ( nextProps.taskid !== this.props.taskid ) {
	    let jwttoken = localStorage.getItem('token').split(" ")[1];
    	    ...
    	    let request = 'http://api.induago.com/list/'+nextProps.taskid;
    	    axios.get(request, {
    		headers: { 'Authorization': "Bearer " + jwttoken }
    	    })
    		.then(function (response) {
	...
	axios({
	    url: 'http://api.induago.com/new/task',
	    method: 'post',
	    headers: { 'Authorization': "Bearer " + jwttoken },
	    data: data
	}).then(function (response) {
	...
	let request = 'http://api.induago.com/list/' + this.props.taskid + '/share';
